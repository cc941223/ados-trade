...
⚠️ **已废弃，不再是默认连接方案**：M0 阶段的 IBKR 连接方式已经从
"IBeam + Client Portal CPAPI"换成"ib_insync + IB Gateway Docker"（见
`brokers/ibkr/client.py` 里的 `IBInsyncClient`，以及仓库根目录
`docker-compose.yml` 里的 `ibgateway` 服务）。这个文件和同目录的
`collector.py` 完整保留下来，仅供万一新方案出问题时回退参考，**不再是
`brokers.ibkr` 的默认导出**，也不会被 `docker-compose.yml` 启动。新代码
不应该再依赖这里的 `IBKRClient`；如果要恢复使用它，需要把
`docker-compose.yml` 换回 `ibeam` 服务，并把调用方改回从
`brokers.ibkr.legacy_ibeam` 导入。
